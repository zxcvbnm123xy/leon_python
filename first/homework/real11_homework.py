...
# 2.编写一个分页显示类，初始化传入记录总数。希望可以通过设置每页记录数和页码，可以显示当前页的信息。其中每页记录数与页码使用property实现。注意，如果页码设置不正确（如<1或者>最大页码），提示错误信息。设计方法能够返回当前页显示的记录区间。
# Page(20)
# pagesize=5
# # 一共应该有4页
# currenPage=6
...
#
# 3.编写电脑类，提供一个方法，能够与移动设备（U盘，MP3，移动硬盘）进行读写交互。如果参数类型不是移动设备的类型，则打印错误信息。MP3除了读与写之外，还额外具有一个播放音乐的功能。
# class Computer：
...
udisk=udisk("udisk")
mp3=Mp3("Mp3")
mobliedisk=MobileDisk("mobliedisk")
c=Computer()
c.connect(udisk)
c.connect(mp3)
c.connect(mobliedisk)
# 4.编写如下的继承结构，类C继承（A，B），类D继承（B，A），类E继承（C，D）或者（D，C），会出现什么情况？
class A:
    pass
class B:
    pass
